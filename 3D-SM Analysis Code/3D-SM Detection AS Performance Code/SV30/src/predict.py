...
class Predictor:
    ...
    Core functions:
    - Load trained ResNet34 image-only model
    ...
    def load_new_data(self, data_dir):
        ...
        # Check if CSV format is correct (should have 2 columns: group ID, SV30 value)
        if data_df.shape[1] < 2:
            raise ValueError(f"CSV file format error, should have at least 2 columns (group ID, SV30 value), actual {data_df.shape[1]} columns")
